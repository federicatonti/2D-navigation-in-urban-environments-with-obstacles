...
class FlowFieldNavEnv(gym.Env):
    ...
    def create_interpolator_for_timestep(self, timestep):
        # Assuming self.flow_field_data['U'] and ['V'] are arrays where each slice [timestep] corresponds to a 2D grid
        u_data_at_timestep = self.flow_field_data['U'][timestep]
        v_data_at_timestep = self.flow_field_data['V'][timestep]

        # Correct extraction of unique X and Y grid points
        unique_x = self.flow_field_data['X'][:, 0]  # Assuming X is structured with unique values down rows
        unique_y = self.flow_field_data['Y'][0, :]  # Assuming Y is structured with unique values across columns
        points = (unique_x, unique_y)

        # Create interpolators using the unique 1D arrays for X and Y
        points = (unique_x, unique_y)
    ...
    def interpolate_velocity(self, position, current_time_step, next_time_step, progression):
        ...
        next_time_step = (current_time_step + 1) % max_time_step  # Ensure wrapping
    ...
    def get_flow_field_velocity(self, position):
        ...
        next_time_step = (self.current_time_step + 1) % self.max_timesteps      #len(self.flow_field_data['U'])
    ...
    def transform_to_flow_relative(self, position, velocity, flow_velocity):
        # Subtract the flow velocity to get the velocity relative to the moving flow
    ...
    def transform_back_to_stationary(self, position, relative_velocity, flow_velocity):
        # Add the flow velocity to convert back to the stationary frame
        absolute_velocity = relative_velocity + flow_velocity
        return position, absolute_velocity
    
    
    def runge_kutta_4th_ang(self, position, velocity, theta, omega, speed, heading, dt):
        def dynamics(pos, vel, ang, ang_vel, spd, hdg):
            u_flow, v_flow = self.get_flow_field_velocity(pos)
            
            # Update the velocity to match the desired speed and heading
            vel[0] = spd * np.cos(hdg) + u_flow
            vel[1] = spd * np.sin(hdg) + v_flow
            
            # Angular dynamics
            dtheta_dt = hdg - ang
            domega_dt = 0  # Assuming no angular acceleration
            
            return vel, vel, hdg, domega_dt
    
        k1_pos, k1_vel, k1_theta, k1_omega = dynamics(position, velocity, theta, omega, speed, heading)
        k2_pos, k2_vel, k2_theta, k2_omega = dynamics(position + 0.5 * k1_pos * dt, velocity + 0.5 * k1_vel * dt, theta + 0.5 * k1_theta * dt, omega, speed, heading)
        k3_pos, k3_vel, k3_theta, k3_omega = dynamics(position + 0.5 * k2_pos * dt, velocity + 0.5 * k2_vel * dt, theta + 0.5 * k2_theta * dt, omega, speed, heading)
        k4_pos, k4_vel, k4_theta, k4_omega = dynamics(position + k3_pos * dt, velocity + k3_vel * dt, theta + k3_theta * dt, omega, speed, heading)
        
        next_position = position + (k1_pos + 2 * k2_pos + 2 * k3_pos + k4_pos) / 6 * dt
        next_velocity = (k1_vel + 2 * k2_vel + 2 * k3_vel + k4_vel) / 6
        next_theta = heading
        next_omega = omega  # No change in angular velocity
    ...
    def step(self, action):
        ...
        for _ in range(40):
            # Directly set the heading and speed
            next_heading = desired_heading
