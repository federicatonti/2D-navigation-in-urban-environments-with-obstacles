...
class PPOTrainer:
    ...
    def compute_gae(self, rewards, values, dones, gamma=0.99, lam=0.95):
        assert rewards.dim() == 3, "rewards should have 3 dimensions (batch_size, seq_len, 1)"
        assert values.dim() == 3, "values should have 3 dimensions (batch_size, seq_len, 1)"
        assert dones.dim() == 3, "dones should have 3 dimensions (batch_size, seq_len, 1)"
...
# Pad and stack trajectories
    ...
    def train(self, env, tasks, meta_optimizer, checkpoint_interval=1, checkpoint_path='checkpoints'):
        episode_count = 0  # Initialize episode count
        
        for epoch in range(self.epochs):
            meta_loss = torch.tensor(0.0, requires_grad=True).to(self.device) 
            
            # Loop over each task
            for task_id, task in enumerate(tasks):
                print(f"Training on task {task_id + 1}/{len(tasks)}")
                ...
                if trajectories is None:
                    continue  # Skip if an empty episode was collected
                
                # Update the model for this task (Inner loop)
                task_loss = self.update(trajectories, self.mini_batch_size)
                
                if task_loss is not None:  # Ensure task_loss is valid
                    meta_loss = meta_loss + task_loss  # Accumulate the loss for meta-update
            
            # Meta-update: Aggregate task losses and update meta-parameters (Outer loop)
            meta_optimizer.zero_grad()
            meta_loss.backward()
            meta_optimizer.step()
            
            print(f"Epoch {epoch+1}/{self.epochs}, Meta Loss: {meta_loss.item()}")
            self.writer.add_scalar("MetaLoss/Epoch", meta_loss.item(), epoch)
            
            # **Evaluate the meta-model periodically**
            if (epoch + 1) % self.eval_interval == 0:
                # **Evaluate on each task separately**
                for task in tasks:
                    env.reset(task=task)
                    avg_reward, std_reward = self.evaluate_policy(env)
                    print(f"Evaluation on task {task}: Average Reward = {avg_reward}, Std = {std_reward}")
                    self.writer.add_scalar(f"Evaluation/Task_{task}/AvgReward", avg_reward, epoch)
                    self.writer.add_scalar(f"Evaluation/Task_{task}/StdReward", std_reward, epoch)
                
                # **Evaluate with task switching**
                avg_reward_switching, std_reward_switching = self.evaluate_policy_with_task_switching(env, tasks)
                print(f"Task Switching Evaluation: Avg Reward = {avg_reward_switching}, Std = {std_reward_switching}")
                self.writer.add_scalar("Evaluation/TaskSwitching/AvgReward", avg_reward_switching, epoch)
                self.writer.add_scalar("Evaluation/TaskSwitching/StdReward", std_reward_switching, epoch)
            
            # Update the episode count
            batch_size = 256
            episode_count += batch_size * len(tasks)
    ...
    def evaluate_policy_with_task_switching(self, env, tasks, episodes=50, max_timesteps=81, random_switching=True):
        total_rewards = []
        ...
        for episode in range(episodes):
            ...
            total_rewards.append(cumulative_reward)
            print(f"Task {task}, Episode {episode + 1}: Total Reward = {cumulative_reward:.2f}")
        
        avg_reward = np.mean(total_rewards)
        std_reward = np.std(total_rewards)
        print(f"Task Switching Evaluation: Avg Reward = {avg_reward:.2f} ± {std_reward:.2f}")
    ...
    def adapt_model_to_task(self, env):
        # Fine-tune the model or adjust the hidden state specifically for this task.
        # This can involve a few gradient steps using the task-specific data.
        print("Adapting model to task...")
        # Add your adaptation logic here, for example:
        # Perform a few gradient steps with task-specific data or reset hidden states.
        return self.model

    def collect_trajectories(self, env, model, batch_size, plot=True, episode_start_count=0, adapt=False):
        trajectories = {
        ...
        for _ in range(batch_size):
            ...
            while not done:
                # Convert to tensor
                obs_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0) 
#                print("Observation tensor shape:", obs_tensor.shape)  # Should print (1, 16)
...
from environment.Meta_env  import FlowFieldNavEnv
...
# Test resetting with different tasks
try:
    observation, info = env.reset(task="obstacle_avoidance")
    print("Reset with obstacle_avoidance task completed successfully.")
except TypeError as e:
    print("Error:", e)

try:
    observation, info = env.reset(task="escape_trap")
    print("Reset with escape_trap task completed successfully.")
except TypeError as e:
    print("Error:", e)

# Define your tasks (as you did in your `reset` method)
tasks = ['obstacle_avoidance', 'escape_trap']

# Initialize the model
model = PPO_LSTM(obs_dim=env.observation_space.shape[0], act_dim=env.action_space.shape[0])

# Define the meta-optimizer
meta_optimizer = optim.Adam(model.parameters(), lr=1e-4)

# Initialize the trainer
trainer = PPOTrainer(model, optimizer=meta_optimizer, clip_ratio=0.2, epochs=1000, sgd_steps=30, gamma=0.99, lam=0.95, mini_batch_size=128, k_epochs=4, eval_interval=5)

# Train the model
trainer.train(env, tasks, meta_optimizer)


checkpoint_dir = 'checkpoints_lin_ang'
