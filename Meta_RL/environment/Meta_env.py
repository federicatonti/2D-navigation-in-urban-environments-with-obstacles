...
class FlowFieldNavEnv(gym.Env):
    def __init__(self):
        ...
        self.trap_region = (-2, -0.5, 0.1, 0.5)  # (x_min, x_max, y_min, y_max)
    ...
    def random_point_before_first_obstacle(self, min_x=-2, max_x=4, min_y=0, max_y=3):
        ...
        start_y = np.random.uniform(0.5, 1.0)
        return np.array([start_x, start_y])
    
    def random_point_in_trapping_region(self, min_x=-2, max_x=4, min_y=0, max_y=3):
        safe_margin = 0.1  
        start_x = np.random.uniform(-2.0, -0.5)
        start_y = np.random.uniform(0.1, 0.5)
        return np.array([start_x, start_y])
    ...
    def random_escape_point(self):
        """
        Generate a random escape point just outside the trapping region.
        """
        x_escape = np.random.uniform(-0.5, -0.25)  # Just outside the trapping region in the x-direction
        y_escape = np.random.uniform(0.5, 1)    # Just outside the trapping region in the y-direction
        return (x_escape, y_escape)
    
    def is_in_trapping_region(self, point):
        x, y = point
        return (-2 < x < -0.25) and (0.2 < y < 0.5)

    def random_start_point(self):
        valid_point = False
        while not valid_point:
            start_x = np.random.uniform(self.domain_x_min, self.domain_x_max)
            start_y = np.random.uniform(self.domain_y_min, self.domain_y_max)
            start_point = np.array([start_x, start_y])
            
            # Check if the generated point is inside any obstacle
            valid_point = not self.check_collision(start_point)
    ...
    def random_target_point(self):
        valid_point = False
        while not valid_point:
            target_x = np.random.uniform(self.domain_x_min, self.domain_x_max)
            target_y = np.random.uniform(self.domain_y_min, self.domain_y_max)
            target_point = np.array([target_x, target_y])
            
            # Check if the generated point is inside any obstacle or problematic region
            valid_point = not self.check_collision(target_point) and not self.is_in_trapping_region(target_point)
            
        return target_point
    

    def calculate_delta_theta(self,uav_position, uav_heading, escape_point):
        """
        Calculate the required change in heading angle (delta_theta) to align the UAV with the escape direction.
    
        Parameters:
        - uav_position: The current position of the UAV as a tuple (x, y).
        - uav_heading: The current heading of the UAV in radians.
        - escape_point: The target point outside the trapping region to which the UAV should move.
    
        Returns:
        - delta_theta: The required change in heading angle (in radians) to align with the escape direction.
        """
    
        # Compute the vector pointing from the UAV's current position to the escape point
        escape_vector = np.array(escape_point) - np.array(uav_position)
        
        # Calculate the desired heading angle based on the escape vector
        theta_desired = np.arctan2(escape_vector[1], escape_vector[0])
        
        # Calculate the difference between the desired heading and the current heading
        delta_theta = theta_desired - uav_heading
    ...
    def compute_min_accelerations(self,flow_field_data, uav_position, uav_heading, escape_point, delta_t):
        """
        Calculate the minimum linear and angular accelerations required for the UAV to escape the trapping region.
    
        Parameters:
        - flow_field_data: Dictionary containing interpolated functions 'U' and 'V' for flow velocities.
        - uav_position: The current position of the UAV as a tuple (x, y).
        - uav_heading: The current heading of the UAV in radians.
        - escape_point: The target point outside the trapping region.
        - delta_t: The time step of the simulation.
    
        Returns:
        - min_linear_acceleration: Minimum required linear acceleration to escape the region.
        - min_angular_acceleration: Minimum required angular acceleration to align with the escape direction.
        """
    
        # Extract UAV position
        x, y = uav_position
        
        # Extract flow field velocities at the UAV's position
        flow_velocity = self.get_flow_field_velocity(uav_position)
        
        u_velocity, v_velocity = flow_velocity
        
        # Compute the magnitude of the flow field velocity
        flow_speed = np.sqrt(u_velocity**2 + v_velocity**2)
        
        # Compute required linear acceleration to counteract the flow field
        min_linear_acceleration = flow_speed / delta_t
        
        # Calculate delta_theta
        delta_theta = self.calculate_delta_theta(uav_position, uav_heading, escape_point)
        
        # Compute required angular acceleration to change direction
        min_angular_acceleration = np.abs(delta_theta) / delta_t
        
        # print("Minimum angular acceleration:", min_angular_acceleration, "Minimum linear acceleration:", min_linear_acceleration)
        
        return min_linear_acceleration, min_angular_acceleration

    def reset(self, seed=None, task=None):
        # Call the superclass reset with only the seed
        ...
        # Set up the environment based on the task
        if task == "obstacle_avoidance":
            self.start_point = self.random_point_before_first_obstacle()
            self.target_point = self.random_point_after_second_obstacle()
        elif task == "escape_trap":
            self.start_point = self.random_point_in_trapping_region()
            self.target_point = self.random_point_after_second_obstacle()
            
        escape_point = self.random_escape_point()
        ...
        min_linear_acceleration, min_angular_acceleration = self.compute_min_accelerations(
        ...
        # Set the UAV state and action limits
        self.max_linear_acceleration = max(min_linear_acceleration, 1.0)
        self.max_angular_acceleration = max(min_angular_acceleration, np.pi / 4)
        ...
        # Define action space based on calculated accelerations
        self.action_space = gym.spaces.Box(
            low=np.array([-self.max_angular_acceleration, -self.max_linear_acceleration]), 
            high=np.array([self.max_angular_acceleration, self.max_linear_acceleration]), 
            dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        # **Adaptive Clipping for Angular Acceleration**
        current_position = self.uav_state[:2]
        
        # Detect if the UAV has passed the first obstacle
        first_obstacle_x_max = self.obstacles[0]['x_range'][1]
        
        # Determine whether to tighten the angular acceleration clipping
        if current_position[0] > first_obstacle_x_max:
            # Tighten the angular acceleration after passing the first obstacle
            adaptive_max_angular_acceleration = self.max_angular_acceleration * 0.1 # Reduce by 50%
            adaptive_max_linear_acceleration = self.max_linear_acceleration * 0.1
        else:
            adaptive_max_angular_acceleration = self.max_angular_acceleration  # Full acceleration before passing the obstacle
            adaptive_max_linear_acceleration = self.max_linear_acceleration
            
        # Apply the adaptive clipping
        angular_acceleration = np.clip(angular_acceleration, -adaptive_max_angular_acceleration, adaptive_max_angular_acceleration)
        linear_acceleration = np.clip(linear_acceleration, -adaptive_max_linear_acceleration, adaptive_max_linear_acceleration)
    ...
    def calculate_reward_and_done(self, speculative_uav_state, action):
        ...
        # Calculate distance-related rewards and penalties
        ...
        safe_distance = 0.3  # Safe distance from obstacles
        r_bar = -self.alpha * np.exp(-self.beta * (average_distance - safe_distance))
        ...
        energy_penalty = -0.3 * np.linalg.norm(propulsion_velocity)
        
        # Penalty for abrupt acceleration changes
        acceleration_change_penalty = -0.1 * (np.abs(linear_acceleration) + np.abs(angular_acceleration))
        
        # Time efficiency penalty
        time_penalty = -0.1  # Small penalty per timestep
        
        # Trap escape reward/penalty
        trap_region = (-2, -0.5, 0.2, 0.5)
        in_trap = (trap_region[0] < current_position[0] < trap_region[1]) and (trap_region[2] < current_position[1] < trap_region[3])
        if in_trap:
            reward -= 10  # Penalty for being in the trap
        else:
            reward += 10  # Reward for escaping the trap
        
        # Target proximity reward
        initial_distance_to_target = np.linalg.norm(self.start_point - self.target_point)
        distance_to_target = np.linalg.norm(current_position - self.target_point)
        reward += 5 * (initial_distance_to_target - distance_to_target)
        
        # Combine all rewards and penalties
        reward += r_trans + r_bar + r_free + P_lin_accel + P_ang_accel + energy_penalty + acceleration_change_penalty + time_penalty + self.r_step
        ...
        if done:
            if self.has_reached_target(current_position):
                self.plot = True
                reward += 100  # Strong reward for reaching the target
                print("Target reached!")
            elif self.is_out_of_bounds(current_position):
                reward -= 20  # Penalty for going out of bounds
            elif self.check_collision(current_position):
                reward -= 100  # Strong penalty for collisions
        
        return reward/1e4, done
